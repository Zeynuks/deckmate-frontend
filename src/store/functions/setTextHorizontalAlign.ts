import {Editor, ObjectType, TextHorizontalAlign, TextObject} from '../types.ts';

export function setTextHorizontalAlign(editor: Editor, newTextAlign: TextHorizontalAlign): Editor {
    ...
    const updatedObjects = slide.objects.map(object => {
        if (editor.selected.objectId.includes(object.id) && object.type === ObjectType.Text) {
            ...
            return {
                ...
                style: {
                    ...textObject.style,
                    textHorizontalAlign: newTextAlign,
                },
            };
        }
        return object;
