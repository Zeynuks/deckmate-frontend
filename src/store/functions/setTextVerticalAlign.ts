import {Editor, ObjectType, TextVerticalAlign, TextObject} from '../types.ts';

export function setTextVerticalAlign(editor: Editor, newTextAlign: TextVerticalAlign): Editor {
    ...
    const updatedObjects = slide.objects.map(object => {
        if (editor.selected.objectId.includes(object.id) && object.type === ObjectType.Text) {
            ...
            return {
                ...
                style: {
                    ...textObject.style,
                    textVerticalAlign: newTextAlign,
                },
            };
        }
        return object;
